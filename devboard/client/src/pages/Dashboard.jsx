        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {recentTasks.length > 0 && (
            <div className="flex items-center gap-1.5 ml-2">
              <span className="text-xs text-[var(--text-secondary)]" title="Recently viewed tasks">🕐</span>
              {recentTasks.map((task) => (
                <button
                  key={task._id}
                  onClick={() => handleSelectTask(task)}
                  className="text-xs bg-[var(--bg-muted)] hover:bg-[var(--border-primary)] text-[var(--text-primary)] border border-[var(--border-primary)] px-2.5 py-0.5 rounded-full truncate max-w-[120px] transition"
                  title={task.title}
                >
                  {task.title}
                </button>
